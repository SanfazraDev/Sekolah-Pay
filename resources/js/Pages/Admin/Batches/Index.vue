...
// Handle search
const handleSearch = () => {
  router.get('/admin/batches', { search: search.value },
    {
      preserveState: true,
      preserveScroll: true,
      only: ['batches'],
...
onMounted(() => {
  setTitle('Angkatan');
  setBreadcrumbs([{ name: 'Angkatan', url: '/admin/batches' }]);
});
...
<template>
    <Head>
        <title>Angkatan - SekolahPay</title>
    </Head>
    
    <div class="container mx-auto px-4 py-6">
        <!-- Header Section -->
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold">Daftar Angkatan</h1>
            <div class="flex space-x-2">
                ...
                <Link href="/admin/batches/create" class="cursor-pointer px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 flex items-center">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Angkatan</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Aksi</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-if="batches.data.length == 0">
                        <td colspan="5" class="px-6 py-4 text-center text-gray-500">Tidak ada data angkatan</td>
                    </tr>
                    <tr v-for="(batch, index) in batches.data" :key="batch?.id || index" class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ index + 1 }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ batch?.title || '-' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex space-x-2">
                                <Link :href="`/admin/batches/edit/${batch.slug}`" class="cursor-pointer text-blue-600 hover:text-blue-900 bg-blue-100 p-2 rounded" :disabled="!batch">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </Link>
                                <button 
                                    @click="batch && confirmDelete(batch)" 
